...
#Eliminando los NA
whale.ahab.m[which(is.na(whale.ahab.m))] <- 0

whale.ahab.m [1:16,]

#calculando la correlación entre whale y ahab, el resultado es una matriz de correlación
cor(whale.ahab.m)
...
cor.data.df <- as.data.frame(whale.ahab.m)
class(cor.data.df)
cor(cor.data.df)

#Al randomizar la columna whale del DF, cada vez que se ejecuta genera una versión randomizada
sample(cor.data.df$whale)

#Calculamos nuevamente la correlación con la versión randomizada
cor(sample(cor.data.df$whale), cor.data.df$ahab)

#Vemos que las correlaciones varian, vamos a almacenar 10000 correlaciones para hacer un mejor análisis
cors.v <- NULL
for (i in 1:10000){
  
  cors.v <- c(cors.v, cor(sample(cor.data.df$whale), cor.data.df$ahab))
...
h<- hist(cors.v, breaks=100, col = "blue", xlab = "Coeficientes de correlación", main="Histogramas de coeficientes de correlación", plot =T)


#Otro método para calcular la correlación
?cor.test
corspers1.v <- cor.test(sample(cor.data.df$whale), cor.data.df$ahab, method="pearson",conf.level = 0.95)
class(corspers1.v)

corspers.v <- NULL
for (i in 1:10000){
  
  corspers.v <- c(corspers.v, cor.test(sample(cor.data.df$whale), cor.data.df$ahab, method="pearson",conf.level = 0.95))
